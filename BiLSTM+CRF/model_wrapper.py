...
from tensorflow_addons.text.crf import crf_log_likelihood  # type: ignore
...
class ModelWithCRFLoss(tf.keras.Model):
    ...
    def __init__(
        self, base_model, sparse_target=True, metric: Union[str, object] = "accuracy"
    ):
        super().__init__()
    ...
    def train_step(self, data):
        ...
        self.metrics_fn.update_state(
            y, viterbi_sequence, tf.sequence_mask(sequence_length, y.shape[1])
        )
        return {
...
class ModelWithCRFLossDSCLoss(tf.keras.Model):
    ...
    def __init__(
        ...
        base_model,
        sparse_target=True,
        metric: Union[str, object] = "accuracy",
        alpha=0.6,
    ):
        super().__init__()
        self.base_model = base_model
    ...
    def compute_loss(self, x, y, sample_weight, training=False):
        ...
        return (
            viterbi_sequence,
            sequence_length,
            tf.reduce_mean(crf_loss),
            tf.reduce_mean(ds_loss),
        )
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = (
                crf_loss + ds_loss + tf.cast(tf.reduce_sum(self.losses), crf_loss.dtype)
            )
        gradients = tape.gradient(loss, self.trainable_variables)
        self.optimizer.apply_gradients(zip(gradients, self.trainable_variables))
        self.loss_tracker.update_state(loss)
